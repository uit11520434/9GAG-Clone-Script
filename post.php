...
if($Next = $mysqli->query("SELECT * FROM media WHERE active=1 and id<$MediaId ORDER BY id DESC LIMIT 1")){
...
if($Submiter = $mysqli->query("SELECT * FROM users WHERE uid='$PostedBy'")){
...
// All comments
if($Comments = $mysqli->query("SELECT * FROM comments LEFT JOIN users ON comments.uid=users.uid WHERE comments.pid='$pid' ORDER BY comments.id DESC")){
