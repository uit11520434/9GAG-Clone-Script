...
<div class="maintitle">Manage Pending Pictures</div>
<div class="clear"></div>
<?php
...
<?php
	...
	*/
	$query = $mysqli->query("SELECT COUNT(*) as num FROM media WHERE active='0' and (type=1 or type=2) ORDER BY id DESC");
	$total_pages = mysqli_fetch_array($query);
	$total_pages = $total_pages['num'];
	
	/* Setup vars for query. */
	$targetpage = "pending_pics.php"; 	//your file name  (the name of this file)
	...
	/* Get data. */
	$result = $mysqli->query("SELECT * FROM media WHERE active='0' and (type=1 or type=2) ORDER BY id DESC LIMIT $start, $limit");
...
<tbody>
    ...
    <td>
	<center>
    <a class="red"href="deleteppics.php?page=<?php echo $page;?>&del=<?php echo $row['id'];?>">Delete</a>
    <a class="blue"href="app.php?page=<?php echo $page;?>&app=<?php echo $row['id'];?>">Approve</a>
...
<?=$pagination?>
<?php
$q = $mysqli->query("SELECT * FROM media WHERE active='0' and (type=1 or type=2) ORDER BY id desc LIMIT $start,$limit");

	$numr = mysqli_num_rows($q);
	if ($numr==0)
	{
	echo '<div class="msg">There are no pending pictures at the moment.</div>';
	}
?>
</div>
</div>
